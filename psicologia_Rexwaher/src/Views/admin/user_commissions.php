...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
    <h1 class="h2">Comisiones de: <?= htmlspecialchars($user['name']) ?></h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        ...
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#assignModal">
            <i class="bi bi-calendar-plus"></i> Asignar Nueva Comisión
        </button>
    </div>
</div>
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        <?php if (empty($history)): ?>
                            <tr><td colspan="5" class="text-center text-muted">Sin historial de comisiones.</td></tr>
                        <?php else: ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Asignar Comisión a Profesional</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="alert alert-warning small">
                    <i class="bi bi-exclamation-triangle"></i> Al asignar una nueva comisión, la actual se cerrará automáticamente el día anterior a la fecha de inicio seleccionada.
                </div>
                <form id="assignForm" onsubmit="assignCommission(event)">
                    <input type="hidden" id="userId" value="<?= $user['id'] ?>">
                    <div class="mb-3">
                        <label class="form-label">Seleccionar Tipo de Comisión</label>
...
<script>
async function assignCommission(e) {
    e.preventDefault();
    const data = {
    ...
    // Endpoint actualizado a /api/admin/commissions/assign
    const res = await fetch('/practicas2026/api/admin/commissions/assign', {
        method: 'POST',
